...
print("Scanning for the 16 well patches...")
for f in tqdm(imgs, desc="Scanning"):
    ...
    if mask is not None and 9 in mask:
        well_files.append((f, mask))

print(f"Found {len(well_files)} well patches. Generating augments...")

aug_count = 0
target_augs_per_img = 65 # 16 * 65 = 1040 augmented images

for f, mask in tqdm(well_files, desc="Augmenting Wells"):
    ...
    for _ in range(target_augs_per_img):
        ...
        new_f = f"aug_well_{aug_count}_{f}"
        cv2.imwrite(os.path.join(image_dir, new_f), aug_img)
        cv2.imwrite(os.path.join(mask_dir, new_f), aug_mask)
        aug_count += 1

print(f"Successfully generated {aug_count} highly-augmented well patches!")
